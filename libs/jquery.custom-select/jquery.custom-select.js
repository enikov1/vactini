...
const CustomSelect = (($) => {
  ...
  class CustomSelect {
    ...
    _select(event) {
	  ...
	  let choice = $choiseBuffer;

	  const values = [...this._values];

    //   const choice = $(event.currentTarget).html();
      ...
      if (!this._options.includeValue) {
		...
		if(this._options.priceOn == true) {
			...
			$.each(this._$options, (i, option) => {
				...
				let priceVal = $option.attr('data-val');
				$option.html(values[i] + '<span class="text-blue">' + price + '</span><span class="text-black">' + priceVal + '</span>');
